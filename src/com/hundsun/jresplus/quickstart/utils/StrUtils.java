...
public class StrUtils {
	...
	public static String tex3cut(String txt, int len) {
		StringBuilder build = new StringBuilder();
		if (txt.length() < len) {
			return txt;
		} else {
			int count = txt.length() / len;
			for (int i = 0; i < count  ; i++) {
				String temp = txt.substring(len * i, (len * i) + len);
				build.append(temp+"\n");
			}
			build.append(txt.substring(count*len));
			return txt3htm(build.toString()); 
		}
	}
	 ...
	 * @return
	 */
	public static String txt3htm(String txt) {
		if (StringUtils.isBlank(txt)) {
			return txt;
		}
		StringBuilder sb = new StringBuilder((int) (txt.length() * 1.2));
